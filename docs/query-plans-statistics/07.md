...
这体现规则证据的正确节奏：单次 PostgreSQL 18.4 实验足以形成候选 runtime check，不足以宣称跨版本普遍阈值。后续复测即使 node type 不同，只要 cardinality 与 partition pruning 的因果关系仍成立，规则就得到进一步确认；若机制变化，就修正 scope/compatibility，而不是让测试强行 grep 旧节点。
