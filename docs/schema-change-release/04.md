...
ch26: operate the full partition lifecycle at production scale
```
...
每个 child index 要验证定义、opclass、collation、validity 与 ownership。对 unique/PK 还要满足 partition key 规则。ch26 将在多分区规模下完整执行。
