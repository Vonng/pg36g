...
action="${1:-all}"
evidence_dir="${PG36_EVIDENCE_DIR:-${PWD}/evidence/ch29/$(date -u +%Y%m%dT%H%M%SZ)}"
...
usage() {
  printf '%s\n' \
    ...
    "capture is L0 read-only; exercise/all are L2 bounded two-cluster disposable-fixture experiments" \
    "no production data, traffic, persistent cluster setting, or real route is touched" \
...
require_exercise() {
  ...
  for evidence_file in \
    "$evidence_dir/remote/migration-evidence.json" \
    "$evidence_dir/remote/route-history.json" \
...
lint_contracts() {
  local lint_dir
  lint_dir="$(mktemp -d "${TMPDIR:-/tmp}/pg36-ch29-lint.XXXXXX")"
  trap 'case "$lint_dir" in "${TMPDIR:-/tmp}"/pg36-ch29-lint.*) rm -rf -- "$lint_dir";; esac' RETURN
  ...
  printf 'declared_counterexamples=29-rejected\n'
  printf 'real_route_change=false\n'
  printf 'production_ch29_gate=pending\n'
}
...
case "$action" in
  ...
  exercise)
    ...
    printf 'two_cluster_fixture_cleanup=verified\n'
    printf 'evidence=%s\n' "$evidence_dir"
    ;;
  verify)
    verify_complete
  ...
  all)
    ...
    printf 'mutation=two-cluster-marker-bound-disposable-fixture-only\n'
    printf 'real_route_change=false\n'
    printf 'evidence=%s\n' "$evidence_dir"
    ;;
esac
