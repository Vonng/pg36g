...
\else
  DO $context_error$
  BEGIN
      RAISE EXCEPTION 'ch13 context guard rejected the database';
  END
  $context_error$;
\endif
...
\else
  DO $context_error$
  BEGIN
      RAISE EXCEPTION 'ch13 context guard rejected a recovery instance';
  END
  $context_error$;
\endif
...
\else
  DO $context_error$
  BEGIN
      RAISE EXCEPTION 'ch13 requires PostgreSQL 14 or later';
  END
  $context_error$;
\endif
...
\else
  DO $context_error$
  BEGIN
      RAISE EXCEPTION 'ch13 session cannot SET ROLE to the object owner';
  END
  $context_error$;
\endif
...
\else
  DO $context_error$
  BEGIN
      RAISE EXCEPTION 'ch13 context guard rejected the effective role';
  END
  $context_error$;
\endif
...
\else
  DO $context_error$
  BEGIN
      RAISE EXCEPTION 'ch13 requires the ch04-v1 model';
  END
  $context_error$;
\endif
...
\else
  DO $context_error$
  BEGIN
      RAISE EXCEPTION 'ch13 requires a constrained pg36_app LOGIN role';
  END
  $context_error$;
\endif
