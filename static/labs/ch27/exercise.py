#!/usr/bin/env python3
"""Upload and run the bounded chapter 27 experiment on the Pigsty meta node."""
...
def main() -> int:
    ...
    if (
        preflight.get("schema") != "pg36-ch27-preflight-evidence-v1"
        ...
        or preflight.get("production_ch27_gate") != "pending"
    ):
    ...
    remote_root = f"/tmp/pg36-ch27.{token}"
    if not re.fullmatch(r"/tmp/pg36-ch27\.[0-9a-f]{32}", remote_root):
    ...
    try:
        ...
        tuning_path = local_remote / "tuning-evidence.json"
        if not tuning_path.is_file():
            raise ExerciseError("remote experiment did not return tuning evidence")
        tuning = json.loads(tuning_path.read_text(encoding="utf-8"))
        if (
            tuning.get("status") != "passed"
            or tuning.get("cleanup", {}).get("database_absent") is not True
            or tuning.get("cleanup", {}).get("role_absent") is not True
            or tuning.get("parameter", {}).get("persistent_configuration_change")
            is not False
        ):
            raise ExerciseError("remote experiment or exact cleanup did not pass")
    finally:
        if re.fullmatch(r"/tmp/pg36-ch27\.[0-9a-f]{32}", remote_root):
            completed = run(
                [
                    "ssh",
                    "-o",
        ...
        cleanup = {
            "schema": "pg36-ch27-remote-cleanup-v1",
            "captured_at": utc_now(),
...
if __name__ == "__main__":
    try:
        ...
        print(f"chapter 27 exercise failed: {exc}", file=sys.stderr)
        raise SystemExit(1)
