...
action="${1:-all}"
evidence_dir="${PG36_EVIDENCE_DIR:-${PWD}/evidence/ch27/$(date -u +%Y%m%dT%H%M%SZ)}"
...
usage() {
  printf '%s\n' \
    ...
    "capture is L0 read-only; exercise/all are L2 bounded session-local parameter experiments" \
    "no persistent PostgreSQL, Patroni, Pigsty, or operating-system setting is changed" \
...
require_exercise() {
  ...
  for evidence_file in \
    "$evidence_dir/remote/tuning-evidence.json" \
...
lint_contracts() {
  local lint_dir
  lint_dir="$(mktemp -d "${TMPDIR:-/tmp}/pg36-ch27-lint.XXXXXX")"
  trap 'case "$lint_dir" in "${TMPDIR:-/tmp}"/pg36-ch27-lint.*) rm -rf -- "$lint_dir";; esac' RETURN
  ...
  printf 'tested_parameters=1\n'
  printf 'planned_runs=10\n'
  printf 'counterexamples=28-rejected\n'
  printf 'persistent_change=false\n'
  printf 'production_ch27_gate=pending\n'
}
...
case "$action" in
  ...
  all)
    ...
    printf 'mutation=marker-bound-fixture-and-session-local-plan-cache-mode-only\n'
    printf 'persistent_change=false\n'
    printf 'evidence=%s\n' "$evidence_dir"
    ;;
esac
