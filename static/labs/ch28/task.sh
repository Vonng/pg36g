...
action="${1:-all}"
evidence_dir="${PG36_EVIDENCE_DIR:-${PWD}/evidence/ch28/$(date -u +%Y%m%dT%H%M%SZ)}"
...
usage() {
  printf '%s\n' \
    ...
    "capture is L0 read-only; exercise/all are L2 bounded disposable-fixture maintenance experiments" \
    "no production data, traffic, or persistent cluster setting is touched" \
...
require_exercise() {
  ...
  for evidence_file in \
    "$evidence_dir/remote/maintenance-evidence.json" \
    "$evidence_dir/remote/events-2024.csv" \
...
lint_contracts() {
  local lint_dir
  lint_dir="$(mktemp -d "${TMPDIR:-/tmp}/pg36-ch28-lint.XXXXXX")"
  trap 'case "$lint_dir" in "${TMPDIR:-/tmp}"/pg36-ch28-lint.*) rm -rf -- "$lint_dir";; esac' RETURN
  ...
  printf 'declared_counterexamples=28-rejected\n'
  printf 'persistent_change=false\n'
  printf 'vacuum_full=false\n'
  printf 'production_ch28_gate=pending\n'
}
...
case "$action" in
  ...
  all)
    ...
    printf 'mutation=marker-bound-disposable-fixture-only\n'
    printf 'persistent_change=false\n'
    printf 'evidence=%s\n' "$evidence_dir"
    ;;
esac
