#!/usr/bin/env python3
"""Upload and run the bounded chapter 28 experiment on the Pigsty meta node."""
...
def main() -> int:
    ...
    if (
        ...
        or preflight.get("target") != "pg36-l2-vagrant/pg-test"
        or preflight.get("clean_start")
        != {"database_absent": True, "role_absent": True}
        or preflight.get("production_ch28_gate") != "pending"
    ):
    ...
    remote_root = f"/tmp/pg36-ch28.{token}"
    if not re.fullmatch(r"/tmp/pg36-ch28\.[0-9a-f]{32}", remote_root):
    ...
    try:
        ...
        if experiment_return_code == 0:
            ...
            # Preserve partial evidence when the remote process managed to write it.
            probe = run(
                [
                    "ssh",
                    "-o",
        ...
        maintenance_path = local_remote / "maintenance-evidence.json"
        if not maintenance_path.is_file():
            raise ExerciseError("remote experiment did not return maintenance evidence")
        maintenance = json.loads(maintenance_path.read_text(encoding="utf-8"))
        if (
            maintenance.get("status") != "passed"
            or maintenance.get("cleanup", {}).get("database_absent") is not True
            or maintenance.get("cleanup", {}).get("role_absent") is not True
            or maintenance.get("safety", {}).get(
                "persistent_configuration_change"
            )
            is not False
        ):
            raise ExerciseError("remote experiment or exact cleanup did not pass")
    finally:
        if re.fullmatch(r"/tmp/pg36-ch28\.[0-9a-f]{32}", remote_root):
            completed = run(
                [
                    "ssh",
                    "-o",
        ...
        cleanup = {
            "schema": "pg36-ch28-remote-cleanup-v1",
            "captured_at": utc_now(),
...
if __name__ == "__main__":
    try:
        ...
        print(f"chapter 28 exercise failed: {exc}", file=sys.stderr)
        raise SystemExit(1)
