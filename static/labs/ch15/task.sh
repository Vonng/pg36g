...
evidence_dir="${PG36_EVIDENCE_DIR:-${PWD}/evidence/ch15/$(date -u +%Y%m%dT%H%M%SZ)}"
admin_connection="service=${pg_service} application_name=pg36-ch15-${action}"
app_connection="service=${pg_service} user=pg36_app application_name=pg36-ch15-app"
reset_worker_pid=""
reset_backend_pid=""
...
usage() {
  printf '%s\n' \
    ...
    "all proves frozen exports, ranking quality, plans, privileges, reset guards, and rebuild" \
    "reset requires PG36_RESET_TOKEN and PG36_RESET_TARGET" \
    "database actions require PGSERVICEFILE and the verified ch14 extensions"
}
    ...
    psql -X -w -qAt \
      ...
      --command="
        ...
        WHERE pid = ${reset_backend_pid}
          ...
          AND application_name = 'pg36-ch15-active-reset';
      " >/dev/null 2>&1 || true
  fi
...
write_manifest() {
  ...
  release_checksum="$(
    canonical_json_checksum \
      "$script_dir/baseline-v1.3-proposal.json"
  )"
  fixture_checksum="$(
    canonical_json_checksum \
...

  {
    ...
    printf 'embedding_model=pg36-handcrafted-topic-4d-v1\n'
...
collect_cycle() {
  ...
  capture_admin_csv \
    "$script_dir/corpus-export.sql" \
    "$output_dir/corpus.csv"
  capture_admin_csv \
    "$script_dir/query-export.sql" \
    "$output_dir/queries.csv"
  capture_admin_csv \
    "$script_dir/judgment-export.sql" \
    "$output_dir/judgments.csv"

  cmp "$script_dir/frozen-corpus.csv" \
      "$output_dir/corpus.csv"
  cmp "$script_dir/frozen-queries.csv" \
      "$output_dir/queries.csv"
  cmp "$script_dir/frozen-judgments.csv" \
      "$output_dir/judgments.csv"

  capture_admin_csv \
    "$script_dir/document-catalog.sql" \
    "$output_dir/document-catalog.csv"
  capture_admin_csv \
    "$script_dir/fts-analysis.sql" \
    "$output_dir/fts-analysis.csv"
  capture_admin_csv \
  ...
  capture_admin_csv \
    "$script_dir/quality-summary.sql" \
    "$output_dir/quality-summary.csv"
  capture_admin_csv \
    "$script_dir/quality-detail.sql" \
    "$output_dir/quality-detail.csv"
  capture_admin_csv \
    "$script_dir/ranking-results.sql" \
    "$output_dir/ranking-results.csv"
  capture_admin_csv \
    "$script_dir/ann-compare.sql" \
    "$output_dir/ann-compare.csv"

  capture_admin_text \
    "$script_dir/fts-plan.sql" \
    "$output_dir/fts-plan.txt"
  capture_admin_text \
    "$script_dir/trigram-plan.sql" \
    "$output_dir/trigram-plan.txt"
  capture_admin_text \
    "$script_dir/vector-exact-plan.sql" \
    "$output_dir/vector-exact-plan.txt"
  capture_admin_text \
    "$script_dir/vector-hnsw-plan.sql" \
    "$output_dir/vector-hnsw-plan.txt"
  capture_admin_text \
    "$script_dir/vector-filtered-plan.sql" \
    "$output_dir/vector-filtered-plan.txt"
  ...
  python3 "$script_dir/review.py" \
    "$output_dir" \
    --baseline "$script_dir/baseline-v1.3-proposal.json" \
...
prove_active_guard() {
  ...
  psql -X -w \
    --dbname="service=${pg_service} application_name=pg36-ch15-active-reset" \
    --set=ON_ERROR_STOP=1 \
...
case "$action" in
  ...
  all)
    ...
    printf 'quality=precision+recall+mrr+ndcg\n'
    printf 'ranking=fts+trigram+exact-vector+rrf\n'
    printf 'ann=q06-exact-vs-hnsw-recall-1.000000\n'
    printf 'guards=P3660+P3661+P3663\n'
    printf 'extensions=ch14-preserved\n'
    printf 'pigsty_l1=not-run\n'
    printf 'evidence=%s\n' "$evidence_dir"
    printf 'release_candidate_checksum=%s\n' \
      "$(canonical_json_checksum \
