...
DO $guard$
BEGIN
    IF current_database() <> 'pg36_capacity'
       OR current_user <> 'dbuser_pg36bench'
       OR pg_is_in_recovery() THEN
        RAISE EXCEPTION
            'chapter 26 cell reset target guard failed: db=%, user=%, recovery=%',
            current_database(),
            current_user,
            pg_is_in_recovery();
    END IF;
    IF NOT EXISTS (
        ...
        WHERE marker = 'pg36-ch26-disposable-capacity-fixture-v1'
    ) THEN
        RAISE EXCEPTION 'chapter 26 fixture marker is missing';
    END IF;
END
$guard$;
